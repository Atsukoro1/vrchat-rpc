...
export const LoginPage = () => {
    ...
    const conditionalRedirect = (requiresTwoFactorAuth: ('otp' | 'totp' | 'emailOtp')[]) => {
    ...
    const onLogin = async (data: CredentialsInput) => {
        try {
            setIsLoading(true);
            const response: CredentialsSuccessResponse = await invoke("user_credentials_login", data);
            setIsLoading(false)
    ...
    return <CredentialsLogin error={error} onSubmit={onLogin} isPending={isLoading} />
}